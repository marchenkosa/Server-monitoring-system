...
def register_endpoints(app, influx_manager, alert_engine):
    ...
    def delete_server(server_id):
        ...
        try:
            ...
            # Удаляем все оповещения для этого сервера
            Alert.query.filter_by(server_id=server.id).delete()
            
            # Данные в InfluxDB остаются, так как удаление по тегам требует отдельного запроса
            # и может быть ресурсоёмким. При необходимости можно добавить удаление позже.
    ...
    def restart_server():
        def _restart():
            ...
            time.sleep(1)
            # В режиме отладки Flask автоматически перезагружается при изменении кода,
            # но мы вызовем принудительный перезапуск через sys.exit.
            # Если сервер запущен через стандартный python app.py, это вызовет завершение.
            # Для production рекомендуется использовать systemd/supervisor, которые перезапустят процесс.
            logging.info("Restarting server...")
            # Закрываем соединения с БД и InfluxDB, если нужно
            # db.session.close()
            # Если используется InfluxDB, можно закрыть клиент
            # if influx_manager and influx_manager.client:
            #     influx_manager.client.close()
            # Завершаем процесс с кодом 42, который можно интерпретировать как сигнал к перезапуску
            sys.exit(42)
        
        # Запускаем перезапуск в фоновом потоке, чтобы сразу вернуть ответ
